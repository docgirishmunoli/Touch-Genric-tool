...
//
// ===============================================================================
// UPDATED VERSION - S19 File Generation & Loading Support
// ===============================================================================
// 
// This file has been updated with the following improvements:
//
// NEW FEATURE: S19 FILE LOADING SUPPORT
// --------------------------------------
// You can now load .s19 files in addition to .hex files!
// - Click "Load Config From File" button
// - Select "S19 Files (*.s19)" from file type dropdown
// - Configuration data is automatically parsed and loaded into widgets
// - Supports S1, S2, and S3 record types (16-bit, 24-bit, 32-bit addresses)
//
// EXISTING FIXES: S19 FILE GENERATION
// ------------------------------------
// Key changes for file generation:
//
//
// NEW METHOD ADDED:
// ----------------
// LoadDataFromS19File(string filePath) - Parses S19 file and returns byte array
//   - Handles S1, S2, S3 record types
//   - Automatically detects address range
//   - Uses PADDING_BYTE_VALUE for gaps in data
//   - Provides detailed error messages
//
// 1. CONFIGURABLE PADDING BYTE (Line ~606)
//    - Added constant: PADDING_BYTE_VALUE = 0xFF
//    - Changed from Array.Clear() (0x00) to explicit 0xFF initialization
//    - Most flash memory devices expect 0xFF for unused areas
//    - Change to 0x00 if your device requires it
//
// 2. FLEXIBLE SIZE CALCULATION (Line ~607)
//    - Added constant: USE_LEGACY_SIZE_CALCULATION = true
//    - When true: Uses original formula (768 bytes total)
//    - When false: Uses exact size without extra padding (676 bytes)
//    - Adjust based on your device requirements
//
// 3. IMPROVED DOCUMENTATION (Line ~1050)
//    - Added comprehensive comments explaining data layout
//    - Shows exact byte positions of each section
//    - Includes debugging output to console
//
// 4. DEBUGGING SUPPORT (Line ~1107)
//    - Prints data size, CRC values, and padding to console
//    - Comment out these lines in production if needed
//
// HOW TO USE:
// -----------
// 1. If S19 files differ from another tool, first try changing PADDING_BYTE_VALUE:
//    - Change line 606 from 0xFF to 0x00 (or vice versa)
//    
// 2. If padding doesn't fix it, try changing size calculation:
//    - Change line 607 USE_LEGACY_SIZE_CALCULATION to false
//    
// 3. Run the debugger and check console output when saving files
//    - Look for the "Generated config data: XXX bytes" message
//    - Compare with the other tool's file size
//
// 4. Use S19FileAnalyzer.cs (provided separately) to compare files:
//    - S19FileAnalyzer.CompareS19Files("other.s19", "yours.s19", "report.txt")
//
// TROUBLESHOOTING:
// ---------------
// - If S19 checksums are invalid: The S19 format is correct, problem is in data
// - If data content differs: Check PADDING_BYTE_VALUE and size calculation
// - If addresses differ: Check S19 start address in the UI
...
namespace TouchCommanderGenericNamespace
{
    ...
    public class ConfigDataModel
    {
        ...
        private const ushort CCITT16_DEFAULT_SEED = 0xFFFF;  // IMPORTANT: Matches embedded code line 272

        // Set this to match your device's endianness
        // true = Little Endian (CRC_LOW=1, CRC_HIGH=0 in C code)
        // false = Big Endian (CRC_LOW=0, CRC_HIGH=1 in C code)
        private const bool USE_LITTLE_ENDIAN = true;  // Set to true for little endian (MICRO_IS_LITTLE_ENDIAN=1)

        // CRC16 CCITT lookup tables - from Crc16.c reference
        private static readonly byte[] Tab_Crc16_Hi = new byte[256] {
        ...
        private static readonly byte[] Tab_Crc16_Lo = new byte[256] {
        ...
        /// <summary>
        /// Process a single byte through CRC16 calculation
        /// Implements Crc16__ByteProcess from Crc16.c with endianness support
        /// </summary>
        private ushort Crc16_ByteProcess(ushort crc_accum, byte data_byte)
        {
            byte crc_low, crc_high;

            if (USE_LITTLE_ENDIAN)
            {
                // Little Endian: CRC_LOW=1, CRC_HIGH=0
                crc_low = (byte)((crc_accum >> 8) & 0xFF);   // byte[1]
                crc_high = (byte)(crc_accum & 0xFF);         // byte[0]

                data_byte ^= crc_low;
                crc_low = (byte)(crc_high ^ Tab_Crc16_Hi[data_byte]);
                crc_high = Tab_Crc16_Lo[data_byte];

                return (ushort)((crc_low << 8) | crc_high);
            }
            else
            {
                // Big Endian: CRC_LOW=0, CRC_HIGH=1
                crc_low = (byte)(crc_accum & 0xFF);          // byte[0]
                crc_high = (byte)((crc_accum >> 8) & 0xFF);  // byte[1]
        ...
        /// <summary>
        /// Calculate CRC16 over a range of bytes
        /// Implements Crc16__RangeProcess from Crc16.c
        /// </summary>
        private ushort CalculateCrc16(byte[] configuration, ushort numOfBytes, ushort seed)
        {
            ...
            {
                crc = Crc16_ByteProcess(crc, configuration[i]);
            }
    ...
    // ========================================================================
    // Generates Motorola S-Record format (S19) files from binary data.
    // 
    // S19 Format Overview:
    // - S3 records: Data records with 32-bit addresses (up to 4GB address space)
    // - S7 record: Termination record with entry point address
    // 
    // Each S3 record format:
    // S3[ByteCount][32-bit Address][Data (up to 16 bytes)][Checksum]
    // 
    // Example: S315FFFF7C3000807DC00070100640000000064008870D
    //          │││  │        │                              ││
    //          │││  │        │                              │└─ Checksum
    //          │││  │        └──────────────────────────────┴─ Data (16 bytes)
    //          │││  └─ Address: FFFF7C30
    //          ││└─ Count: 0x15 = 21 (4 addr + 16 data + 1 checksum)
    //          │└─ S3 = 32-bit address record
    //          └─ S = Record start
    //
    // This implementation:
    // - Uses 16 bytes per line (standard practice)
    // - Generates S3 records for all data
    // - Adds S7 termination record with entry point 0x00000000
    // - Calculates correct checksums (one's complement of sum of all bytes)
    //
    // The S19 format itself is CORRECT. If your S19 files differ from another
    // tool, the issue is in the SOURCE DATA being written, not the S19 format.
    // ========================================================================
    ...
    public static class S19FileGenerator
    {
        public static void SaveS19File(string fileName, byte[] data, UInt32 startAddress)
        {
            ...
            using (StreamWriter writer = new StreamWriter(s19FileName, false, Encoding.ASCII))
            {
                ...
                // Write S7 termination record - use 0x00000000 as entry point (standard)
                // The S7 record specifies the entry/execution address, typically 0x00000000
                string terminationRecord = GenerateS7Record(0x00000000);
        ...
        private static string GenerateS7Record(UInt32 entryAddress)
        {
            ...
            sb.Append("S7");
            sb.Append(byteCount.ToString("X2"));
            sb.Append(entryAddress.ToString("X8"));

            byte checksum = CalculateChecksum(byteCount, entryAddress, new byte[0]);
            sb.Append(checksum.ToString("X2"));
    ...
    public class TouchConfigTabPage : TouchCommanderTabBase
    {
        ...
        // Configuration constants for file generation  
        // IMPORTANT: Adjust these values to match your device requirements
        private const byte PADDING_BYTE_VALUE = 0x00;  // Use 0xFF for flash memory (change to 0x00 if needed)
        private const bool USE_LEGACY_SIZE_CALCULATION = true;  // Set to false to use exact size (cap+dev+crc only)
        ...
        private void initView(object data)
        {
            ...
            lbS19StartAddress.Text = "S19 Start Addrss(0x Hex):";
        ...
        /// <summary>
        /// Load binary data from S19 file format
        /// Parses S19 records and extracts data into a byte array
        /// </summary>
        private byte[] LoadDataFromS19File(string filePath)
        {
            try
            {
                ...
                Dictionary<uint, byte[]> dataMap = new Dictionary<uint, byte[]>();
                ...
                foreach (string line in lines)
                {
                    ...
                    // We only care about data records (S1, S2, S3)
                    if (recordType != "S1" && recordType != "S2" && recordType != "S3")
                        continue;

                    // Parse the record
                    byte byteCount = Convert.ToByte(line.Substring(2, 2), 16);

                    // Determine address length based on record type
                    int addressLength = 0;
                    switch (recordType)
                    {
                        case "S1": addressLength = 4; break;  // 16-bit
                        case "S2": addressLength = 6; break;  // 24-bit
                        case "S3": addressLength = 8; break;  // 32-bit
                    }

                    uint address = Convert.ToUInt32(line.Substring(4, addressLength), 16);

                    // Calculate data length: byteCount - addressBytes - checksumByte
                    int dataLength = byteCount - (addressLength / 2) - 1;

                    if (dataLength > 0)
                    {
                        string dataHex = line.Substring(4 + addressLength, dataLength * 2);
                        ...
                        // Store data with its address
                        dataMap[address] = data;

                        // Track min/max addresses
                        if (address < minAddress)
                            minAddress = address;

                        uint endAddress = address + (uint)dataLength;
                        if (endAddress > maxAddress)
                            maxAddress = endAddress;
                    }
                }

                // Second pass: build contiguous byte array
                if (dataMap.Count == 0)
                {
                    throw new Exception("No data records found in S19 file");
                }

                // Create array from minAddress to maxAddress
                int totalSize = (int)(maxAddress - minAddress);
                byte[] result = new byte[totalSize];

                // Initialize with padding value (match what we use for saving)
                for (int i = 0; i < result.Length; i++)
                {
                    result[i] = PADDING_BYTE_VALUE;
                }
                ...
                foreach (var kvp in dataMap)
                {
                    ...
                    byte[] data = kvp.Value;
                    int offset = (int)(address - minAddress);

                    Array.Copy(data, 0, result, offset, data.Length);
                }

                Console.WriteLine($"Loaded S19 file: {result.Length} bytes (address range: 0x{minAddress:X8} - 0x{maxAddress:X8})");

                return result;
            }
            catch (Exception ex)
            {
                throw new Exception($"Error loading S19 file: {ex.Message}", ex);
            }
        }
        ...
        private void btnLoadFromFile_Click(object sender, System.EventArgs e)
        {
            if (cbTouchChipType.Text == "Rx130" || cbTouchChipType.Text == "Rx130INT")
            {
                ...
                openFileDialog.Filter = "HEX Files (*.hex)|*.hex|S19 Files (*.s19)|*.s19|txt files (*.txt)|*.txt|All files (*.*)|*.*";
                openFileDialog.FilterIndex = 1;  // Default to HEX files
                ...
                {
                    // Load S19 file format
                    try
                    {
                        config_data = LoadDataFromS19File(file);
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show($"Error loading S19 file: {ex.Message}\n\nPlease ensure the file is a valid Motorola S-Record format.",
                                      "S19 Load Error",
                ...
                else
                {
                    ...
                    config_data = StringToByteArray(config_text);
                }

                TouchConfigModel model = (TouchConfigModel)control.GetModel("Touch Config");
                if (model != null)
                {
                    // Use config_data.Length directly (works for both HEX and S19)
                    int cfg_data_length = config_data.Length;
                    ...
                    if (fileExtension == ".s19")
                    {
                        MessageBox.Show($"S19 file loaded successfully!\n\n" +
                                      $"File: {Path.GetFileName(file)}\n" +
                                      $"Data size: {cfg_data_length} bytes\n\n" +
                                      "Configuration has been updated in all widgets.",
                                      "S19 Load Success",
                                      MessageBoxButtons.OK,
            ...
            if (cbTouchChipType.Text == "Rx130" || cbTouchChipType.Text == "Rx130INT")
            {
                ...
                if (model != null)
                {
                    ...
                    if (cbTouchChipType.Text == "Rx130" || cbTouchChipType.Text == "Rx130INT")
                    {
                        ...
                        // ========================================================================
                        // DATA ARRAY PREPARATION FOR FILE GENERATION
                        // ========================================================================
                        // This section creates the binary data array that will be written to
                        // .hex, .bin, and .s19 files. The layout must match the device's expected format.
                        //
                        // Current layout:
                        // [0..511]      : cap_cfg (512 bytes) - Capacitive sensing configuration
                        // [512..671]    : dev_cfg (160 bytes) - Device configuration  
                        // [672..673]    : dev_cfg CRC (2 bytes, little-endian)
                        // [674..675]    : cap_cfg CRC (2 bytes, little-endian)
                        // [676..767]    : Padding (if USE_LEGACY_SIZE_CALCULATION = true)
                        //
                        // Total size: cap_cfg_size + dev_cfg_size + 4 (CRCs) + padding
                        // ========================================================================

                        int total_data_size;
                        if (USE_LEGACY_SIZE_CALCULATION)
                        {
                            // Legacy calculation: adds (256 - dev_cfg_size) bytes of padding
                            // With dev_cfg_size=160, this adds 96 bytes, totaling 768 bytes
                            total_data_size = dev_cfg_size + cap_cfg_size + (256 - dev_cfg_size);
                        }
                        else
                        {
                            // Exact size: just cap_cfg + dev_cfg + 4 bytes for CRCs (no extra padding)
                            // Total would be 512 + 160 + 4 = 676 bytes
                            total_data_size = cap_cfg_size + dev_cfg_size + 4;
                        }

                        byte[] data = new byte[total_data_size];

                        // Initialize array with padding value
                        // IMPORTANT: Use 0xFF for flash memory (typical), or 0x00 if device requires it
                        // You can change PADDING_BYTE_VALUE constant at the top of the class
                        for (int idx = 0; idx < data.Length; idx++)
                        {
                            data[idx] = PADDING_BYTE_VALUE;
                        }

                        // Copy capacitive configuration data
                        ...
                        // Add device configuration CRC (little-endian: LSB first, MSB second)
                        ...
                        // Add capacitive configuration CRC (little-endian: LSB first, MSB second)
                        ...
                        Console.WriteLine($"Generated config data: {data.Length} bytes");
                        Console.WriteLine($"  Cap Config: {cap_cfg_size} bytes");
                        Console.WriteLine($"  Dev Config: {dev_cfg_size} bytes");
                        Console.WriteLine($"  Dev CRC: 0x{model.getDevCfgCRC(iSelectedSlaveId):X4}");
                        Console.WriteLine($"  Cap CRC: 0x{model.getCapCfgCRC(iSelectedSlaveId):X4}");
                        Console.WriteLine($"  Padding: 0x{PADDING_BYTE_VALUE:X2}");
